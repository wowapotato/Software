...
def trans_bin(path1, path2):
    ...
    wv_from_text.init_sims(replace=True)   #初始化词向量模型
    wv_from_text.save(path2)               #保存为二进制文件
...
def get_new_dict(type_vec_path, type_word_path, final_vec_path, final_word_path):
    ...
    #创建词典列表
    # 其中0 PAD_ID, 1 SOS_ID, 2 EOS_ID, 3 UNK_ID
...
# 根据文本内容和词典，获取文本中每个词在词典中的索引
def get_index(type, text, word_dict):
    location = []
    #如果文本是代码类型，则在索引列表的开始添加特殊标记
    if type == 'code':
        location.append(1)
        len_c = len(text)
        if len_c + 1 < 350:
    ...
    #如果文本长度超过特定阈值，则截断或填充索引列表，以适应预定义的长度
        if len(text) == 0:
            location.append(0)
        elif text[0] == '-10000':
            location.append(0)
